...
const normalizeCategoryOptions = (rawValue) => {
    ...
    for (const entry of rawValue) {
        ...
        const candidateName = typeof entry.name === 'string'
            ? entry.name.trim()
            : '';
...
const chooseCategoryId = (categories, keywords, fallback) => {
...
const fallbackClassification = (description, categories) => {
...
const callGemini = async (apiKey, requestBody, model = DEFAULT_GEMINI_MODEL) => {
    if (!apiKey) {
    ...
    try {
        const response = await fetch(`${GEMINI_API_BASE}/models/${encodeURIComponent(normalizedModel)}:generateContent`, {
            method: 'POST',
            headers: {
...
const parseClassification = (text, categories, fallback) => {
...
const buildChatFallback = (question) => {
    return [
...
export const classifyExpense = onCall(callableOptions, async (request) => {
    ...
    const categories = normalizeCategoryOptions(request.data.categories);
...
export const financeChat = onCall(callableOptions, async (request) => {
    ...
    const messages = normalizeChatMessages(request.data.messages);
    const financialContext = typeof request.data.financialContext === 'string'
        ? (request.data.financialContext ?? '').slice(0, 6000)
        : '';
