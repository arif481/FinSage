...
export const CommandPalette = () => {
  ...
  const navActions: Action[] = useMemo(
    () => [
      ...
      {
        id: 'splits',
        label: 'Go to Split Expenses',
        icon: '🔀',
        onSelect: () => navigate('/splits'),
      },
      {
        id: 'recurring',
      ...
      { id: 'spaces', label: 'Go to Spaces', icon: '🤝', onSelect: () => navigate('/spaces') },
      {
        id: 'categories',
      ...
      },
      {
        ...
        icon: '➕',
        onSelect: () => navigate('/transactions?new=transaction'),
      },
      {
        id: 'new-budget',
        label: 'Add Budget',
        icon: '➕',
        onSelect: () => navigate('/budgets?new=budget'),
      },
      {
        id: 'new-loan',
        label: 'Add Loan',
        icon: '➕',
        onSelect: () => navigate('/loans?new=loan'),
      },
      {
        id: 'new-goal',
        label: 'Add Savings Goal',
        icon: '➕',
        onSelect: () => navigate('/goals?new=goal'),
      },
      {
        id: 'new-split',
        label: 'Add Split Expense',
        icon: '➕',
        onSelect: () => navigate('/splits?new=split'),
      },
      {
        id: 'new-recurring',
        label: 'Add Recurring Rule',
        icon: '➕',
        onSelect: () => navigate('/recurring?new=rule'),
      },
      {
        id: 'new-category',
        label: 'Add Category',
        icon: '➕',
        onSelect: () => navigate('/categories?new=category'),
      },
      {
        id: 'new-space',
        label: 'Create Space',
        icon: '➕',
        onSelect: () => navigate('/spaces?new=space'),
      },
    ],
    [navigate],
  )

  const transactionResults: Action[] = useMemo(
    () =>
      query.length > 2
        ? transactions
            .filter(
              (t) =>
                fuzzyMatch(query, t.description) || t.tags.some((tag) => fuzzyMatch(query, tag)),
            )
            .slice(0, 5)
            .map((t) => ({
              id: t.id,
