  ...
  return (
    <main className="screen stack">
      <header className="screen-header" style={{ animation: 'fade-up 400ms ease both' }}>
        <div>
          ...
          <p className="section-subtitle">
            Customize, add, merge, or delete categories for your transactions.
          </p>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button
            ...
            type="button"
            onClick={() => setMergeSource(mergeSource ? null : (categories[0]?.id ?? null))}
          >
      ...
      {mergeSource && (
        <section
          className="card"
          style={{ animation: 'fade-up 300ms ease both', borderLeft: '4px solid var(--primary)' }}
        >
          <h3>🔗 Merge categories</h3>
          <p className="section-subtitle" style={{ marginBottom: '1rem' }}>
            All transactions from the source will be reassigned to the target. The source category
                ...
                {COLOR_OPTIONS.map((clr) => (
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      background: clr,
                      border: color === clr ? '3px solid var(--text)' : '3px solid transparent',
                      transition: 'all 0.2s',
                    }}
                  />
                ))}
