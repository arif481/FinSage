...
export const SpaceTransactionsScreen = () => {
  ...
  useQuickCreateParam(
    ...
    { enabled: Boolean(space && user), scrollToId: 'space-transaction-form' },
  )
  ...
  const toggleSplitMember = (uid: string) => {
    setSplitAmong((prev) => (prev.includes(uid) ? prev.filter((u) => u !== uid) : [...prev, uid]))
  }
  ...
  // Per-member spending
  const memberSpending = space.members
    .map((m) => {
    ...
    <main className="screen stack">
      ...
      <section
        style={{
          ...
          gap: '0.8rem',
          flexWrap: 'wrap',
          animation: 'fade-up 400ms ease both 100ms',
        }}
      >
          ...
          >
            <div className="field-row">
              ...
              <label className="field">
                ...
                <select
                  value={type}
                  onChange={(e) => setType(e.target.value as SpaceTransactionType)}
                >
        ...
        <section className="stack" style={{ gap: '0.5rem' }}>
          {filtered.map((txn, i) => (
            <article
              key={txn.id}
              className="card"
              style={{ '--stagger': i + 1, padding: '0.8rem 1rem' } as React.CSSProperties}
            >
              <div
                style={{
                  display: 'flex',
