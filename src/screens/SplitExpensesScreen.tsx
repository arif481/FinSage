    ...
    <main className="screen stack">
      ...
      <header className="screen-header" style={{ animation: 'fade-up 400ms ease both' }}>
        <div>
          ...
          <p className="section-subtitle">Track shared expenses and settle up with friends.</p>
        </div>
        <button
            ...
            >
              ...
              >
                <div>
                  ...
                  <small style={{ color: 'var(--text-muted)' }}>
                    with {split.splitWith} • Total: {formatCurrency(split.totalAmount, currency)} •
                    Your share: {formatCurrency(split.myShare, currency)}
                  </small>
                </div>
              ...
              <div className="button-row" style={{ marginTop: '0.75rem' }}>
                <button
                  ...
                  onClick={() => void handleSettle(split.id)}
                >
                  ✅ Settle
                </button>
                <button
        ...
        <section className="stack" style={{ gap: '1rem' }}>
          ...
          {settled.map((split) => (
            <article
              key={split.id}
              className="card"
              style={{ borderLeft: '4px solid var(--success)', opacity: 0.75 }}
            >
              <div
              ...
              >
                <div>
                  ...
                  <small style={{ color: 'var(--text-muted)' }}>
                    with {split.splitWith} • {formatCurrency(split.totalAmount, currency)}
                  </small>
                </div>
                <button
                  className="secondary-button"
      ...
      {splitExpenses.length === 0 && !showForm && (
        <section className="card" style={{ textAlign: 'center', padding: '3rem 1rem' }}>
          ...
          <p className="section-subtitle">
            Split bills, dinners, and shared costs with friends — track who owes what.
          </p>
          <button
            className="primary-button"
            type="button"
