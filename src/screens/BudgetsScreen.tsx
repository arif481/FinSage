...
export const BudgetsScreen = () => {
  ...
  useQuickCreateParam('budget', () => undefined, { scrollToId: 'budget-planner' })
  ...
  const approachingCount = progress.filter(
  ...
  const prevOverBudget = prevProgress.reduce(
    (sum, item) => sum + Math.abs(Math.min(item.remaining, 0)),
    0,
  )
  ...
  const handleAiSuggest = () => {
    ...
    if (suggestions.length > 0) {
      ...
      showToast(
        'Not enough spending data for suggestions. Track at least 1 month of expenses.',
        'error',
      )
    }
  }
  ...
  const getCategoryColor = (catId: string) =>
    categories.find((c) => c.id === catId)?.color ?? '#8f95a1'
  ...
  return (
    <main className="screen stack">
        ...
        {insightData.map((item, i) => (
          ...
          >
            ...
            <strong
              style={{
                color:
                  item.label === 'Status' && overspentCount > 0
                    ? 'var(--danger)'
                    : item.label === 'Status' && approachingCount > 0
                      ? 'var(--warning, orange)'
                      : undefined,
              }}
            >
      ...
      {prevProgress.length > 0 && (
        <section
          className="card"
          style={{ '--stagger': 0, animation: 'fade-up 400ms ease both' } as React.CSSProperties}
        >
          ...
          <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
            <div>
              ...
              <p
                style={{ fontWeight: 700, color: 'var(--success)', fontSize: '1.1rem', margin: 0 }}
              >
            ...
            <div>
              ...
              <p
                style={{
                  ...
                  fontSize: '1.1rem',
                  margin: 0,
                  color: prevUnderBudget - prevOverBudget >= 0 ? 'var(--success)' : 'var(--danger)',
                }}
              >
      ...
      <section className="card stack" style={{ '--stagger': 1 } as React.CSSProperties}>
        ...
        <p className="section-subtitle">
          Visualize how each category's spending compares to its budget limit.
        </p>
