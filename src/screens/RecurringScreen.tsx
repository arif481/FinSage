...
export const RecurringScreen = () => {
  ...
  const insightData = [
    ...
    {
      label: 'Net recurring',
      value: formatCurrency(Math.round(totalMonthlyIncome - totalMonthlyExpense), currency),
    },
  ]
    ...
    <main className="screen stack">
      ...
      <header className="screen-header" style={{ animation: 'fade-up 400ms ease both' }}>
        <div>
          ...
          <p className="section-subtitle">Automate repeating income and expenses.</p>
        </div>
        <button
              ...
              >
                <div>
                  ...
                  <small style={{ display: 'block', color: 'var(--text-muted)' }}>
                    {frequencyLabel(rule.frequency)} • {getCategoryName(rule.categoryId)}
                  </small>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <span
                    style={{
                      fontWeight: 600,
                      color: rule.type === 'income' ? 'var(--success)' : 'var(--text)',
                    }}
                  >
                    {rule.type === 'income' ? '+' : '-'}
        ...
        <section className="stack" style={{ gap: '1rem' }}>
          ...
          {pausedRules.map((rule) => (
            <article
              key={rule.id}
              className="card"
              style={{ opacity: 0.7, borderLeft: '4px solid var(--text-muted)' }}
            >
              <div
              ...
              >
                <div>
                  ...
                  <small style={{ color: 'var(--text-muted)' }}>
                    {frequencyLabel(rule.frequency)} • {formatCurrency(rule.amount, currency)}
                  </small>
                </div>
                <div className="button-row">
                  <button
      ...
      {recurringRules.length === 0 && !showForm && (
        <section className="card" style={{ textAlign: 'center', padding: '3rem 1rem' }}>
          ...
          <p className="section-subtitle">
            Set up automatic tracking for subscriptions, rent, salary, and other repeating
            transactions.
          </p>
          <button
            className="primary-button"
