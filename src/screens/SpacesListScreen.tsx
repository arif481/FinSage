...
export const SpacesListScreen = () => {
  ...
  const quickJoin = searchParams.get('join')
  ...
  useEffect(() => {
    if (quickNew !== 'space' && quickJoin !== '1') return
    ...
    window.requestAnimationFrame(() => {
      document
        .getElementById(quickNew === 'space' ? 'space-create-form' : 'space-join-form')
        ?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    })
  }, [quickJoin, quickNew, searchParams, setSearchParams])
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (blockedOrOffline) {
        showToast(
          'Join failed: network/ad-blocker blocked Firebase. Disable blocker for this site and try again.',
          'error',
        )
      } else {
      ...
      <header className="screen-header" style={{ animation: 'fade-up 400ms ease both' }}>
        <div>
          ...
          <p className="section-subtitle">
            Collaborative workspaces for shared finances — split bills, track loans, manage EMIs
            together.
          </p>
        </div>
      ...
      {showJoin && (
        ...
        >
          ...
          >
            ...
            <button className="secondary-button" type="button" onClick={() => setShowJoin(false)}>
              Cancel
            </button>
          </form>
        </section>
        ...
        >
          ...
          >
            <label className="field">
              ...
              <input
                type="text"
                placeholder="e.g. Roommates, Trip Fund, Family"
                value={name}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                {SPACE_ICONS.map((ic) => (
                  <button
                    ...
                    onClick={() => setIcon(ic)}
                    className={
                      icon === ic ? 'space-icon-btn space-icon-btn--active' : 'space-icon-btn'
                    }
                  >
                    {ic}
                  </button>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                {SPACE_COLORS.map((c) => (
                  <button
                    ...
                    onClick={() => setColor(c)}
                    className={
                      color === c ? 'space-color-btn space-color-btn--active' : 'space-color-btn'
                    }
                    style={{ background: c }}
                  />
                ))}
      ...
      {spaces.length > 0 ? (
            ...
            >
              <div className="space-card__header">
                ...
                <div>
                  ...
                  {space.description && <p className="space-card__desc">{space.description}</p>}
                </div>
              </div>
        ...
        <section className="card" style={{ textAlign: 'center', padding: '3rem 1rem' }}>
          ...
          <p
            ...
            style={{ maxWidth: '420px', margin: '0.5rem auto 1.5rem' }}
          >
            Create a shared workspace to track expenses, loans, and EMIs with friends, family, or
            roommates.
          </p>
          <div className="button-row" style={{ justifyContent: 'center' }}>
            <button className="secondary-button" type="button" onClick={() => setShowJoin(true)}>
              🔗 Join with code
            </button>
