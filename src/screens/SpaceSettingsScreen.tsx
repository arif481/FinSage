...
export const SpaceSettingsScreen = () => {
  ...
  useQuickCreateParam(
    ...
    { enabled: Boolean(space), scrollToId: 'space-reminder-form' },
  )
  ...
  const saveCat = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCat) {
        await updateSpaceCategory(spaceId, editingCat.id, {
          name: catName.trim(),
          icon: catIcon,
          color: catColor,
        })
        ...
        await addSpaceCategory(spaceId, {
          name: catName.trim(),
          icon: catIcon,
          color: catColor,
          createdBy: user.uid,
    ...
    <main className="screen stack">
      ...
      <nav className="space-nav" style={{ animation: 'fade-up 300ms ease both' }}>
        ...
        <NavLink
          ...
          className={({ isActive }) => (isActive ? 'space-nav__link active' : 'space-nav__link')}
        >
          📊 Dashboard
        </NavLink>
        <NavLink
          to={`/spaces/${spaceId}/transactions`}
          className={({ isActive }) => (isActive ? 'space-nav__link active' : 'space-nav__link')}
        >
          💸 Transactions
        </NavLink>
      ...
      <section className="card stack" style={{ '--stagger': 0 } as React.CSSProperties}>
        ...
        {editingSpace ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
            ...
            <div>
              <span style={{ fontWeight: 700, display: 'block', marginBottom: '0.4rem' }}>
                Icon
              </span>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                {SPACE_ICONS.map((ic) => (
                  <button
                    ...
                    className={
                      spaceIcon === ic ? 'space-icon-btn space-icon-btn--active' : 'space-icon-btn'
                    }
                  >
                    {ic}
                  </button>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                {SPACE_COLORS.map((c) => (
                  <button
                    ...
                    className={
                      spaceColor === c
                        ? 'space-color-btn space-color-btn--active'
                        : 'space-color-btn'
                    }
                    style={{ background: c }}
                  />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                ...
                <div>
                  <span style={{ fontWeight: 600 }}>
                    {m.displayName}
                    {m.uid === user?.uid && (
                      <small style={{ color: 'var(--text-muted)', marginLeft: '0.3rem' }}>
                        (you)
                      </small>
                    )}
                  </span>
        ...
        {showCatForm && (
          ...
          >
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
                {CAT_ICONS.map((ic) => (
                  <button
                    ...
                    className={
                      catIcon === ic ? 'space-icon-btn space-icon-btn--active' : 'space-icon-btn'
                    }
            ...
            <div className="button-row">
              ...
              <button className="secondary-button" type="button" onClick={resetCatForm}>
                Cancel
              </button>
            </div>
          </form>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
          {categories.map((cat) => (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => void deleteSpaceCategory(spaceId!, cat.id)}
                style={{
                  background: 'none',
                  border: 'none',
                  cursor: 'pointer',
        ...
        {showReminderForm && (
          ...
          >
            <label className="field">
              ...
              <input
                type="text"
                placeholder="e.g. Pay EMI, Settle loan"
                value={reminderTitle}
            ...
            <div className="field-row">
              ...
              <label className="field">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setReminderFrequency(e.target.value as 'once' | 'daily' | 'weekly' | 'monthly')
                  }
                >
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                {space.members.map((m) => (
                  <button
                    ...
                    type="button"
                    className={
                      reminderAssignedTo.includes(m.uid)
                        ? 'space-member-chip space-member-chip--active'
                        : 'space-member-chip'
                    }
                    onClick={() =>
            ...
            <div className="button-row">
              ...
              <button className="secondary-button" type="button" onClick={resetReminderForm}>
                Cancel
              </button>
            </div>
          </form>
                  ...
                  <div style={{ display: 'flex', gap: '0.3rem' }}>
                    {!r.isDismissed && (
                      <button
                        ...
                        onClick={() => void dismissReminder(spaceId!, r.id)}
                      >
                        Dismiss
                      </button>
                    )}
                    <button
                      ...
                      onClick={() => void deleteSpaceReminder(spaceId!, r.id)}
                    >
                      🗑
                    </button>
                  </div>
      ...
      <section
        className="card"
        style={{ '--stagger': 5, borderLeft: '4px solid var(--danger)' } as React.CSSProperties}
      >
